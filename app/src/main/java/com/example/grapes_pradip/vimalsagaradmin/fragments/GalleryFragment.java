...
import com.example.grapes_pradip.vimalsagaradmin.activities.gallery.AddGalleryCategoryActivity;
import com.example.grapes_pradip.vimalsagaradmin.adapters.gallery.RecyclerGalleryCategoryAdapter;
...
public class GalleryFragment extends Fragment implements View.OnClickListener {
    ...
    private RecyclerView recyclerView_gallery_category;
    private LinearLayoutManager linearLayoutManager;
    private ArrayList<GalleryCategoryItem> galleryCategoryItems = new ArrayList<>();
    private RecyclerGalleryCategoryAdapter recyclerGalleryCategoryAdapter;
    ...
    private void refreshContent() {
        galcatid.clear();
        swipe_refresh_information.setRefreshing(false);
        new GetAllGalleryCategory().execute();
    }

    private void findID() {
        swipe_refresh_information = (SwipeRefreshLayout) rootview.findViewById(R.id.swipe_refresh_information);
        recyclerView_gallery_category = (RecyclerView) rootview.findViewById(R.id.recyclerView_gallery_category);
        recyclerView_gallery_category.setLayoutManager(linearLayoutManager);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.txt_addnew:
                Intent intent = new Intent(getActivity(), AddGalleryCategoryActivity.class);
...
//                            Toast.makeText(activity, "You clicked on NO", Toast.LENGTH_SHORT).show();
    ...
    private class GetAllGalleryCategory extends AsyncTask<String, Void, String> {
        String responseJSON = "";
...
//            progressDialog.setCancelable(false);
            ...
            recyclerView_gallery_category.setVisibility(View.GONE);
        }

        @Override
        protected String doInBackground(String... params) {
            responseJSON = JsonParser.getStringResponse(CommonURL.Main_url + CommonAPI_Name.getallcategorygallery);
            return responseJSON;
        }
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                ...
                galleryCategoryItems = new ArrayList<>();
...
//            }
            ...
            recyclerView_gallery_category.setVisibility(View.VISIBLE);
            if (recyclerView_gallery_category != null) {
                recyclerGalleryCategoryAdapter = new RecyclerGalleryCategoryAdapter(getActivity(), galleryCategoryItems);
                if (recyclerGalleryCategoryAdapter.getItemCount() != 0) {
                    recyclerView_gallery_category.setVisibility(View.VISIBLE);
                    img_nodata.setVisibility(View.GONE);
                    recyclerView_gallery_category.setAdapter(recyclerGalleryCategoryAdapter);
...
//            progressDialog.setCancelable(false);
        ...
        @Override
        protected String doInBackground(String... params) {
            for (int i = 0; i < galcatid.size(); i++) {
                String value = galcatid.get(i);
                responseJSON = JsonParser.getStringResponse(CommonURL.Main_url + CommonAPI_Name.muldeletecategorygallery + "?cid=" + value);
            }
            return responseJSON;
        }
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                ...
                if (jsonObject.getString("status").equalsIgnoreCase("success")) {
                    ...
                    Toast.makeText(getActivity(), "Gallery category not deleted.", Toast.LENGTH_SHORT).show();
                }
            } catch (JSONException e) {
                e.printStackTrace();
            }
